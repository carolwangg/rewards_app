...
import { getCustomerCard } from '@/services/apiCalls';
...
export default function ItemMinimal({userId, reward}: Props) {
  ...
  useEffect(()=>{
    getCustomerCard(userId, reward.business_id).then(data =>{
      if (data.user){
        setCustomerPoints(data.user.points);
      }      
    });
  }, [])
  ...
  const addToCartView = useCallback((points: number, cardPoints: number) => {
    console.log("card view")
              console.log("points:"+points+", "+cardPoints)

    if (cardPoints >= points){
          console.log("points satisfied:"+points+", "+cardPoints)
  ...
  return (
    <View testID={"168:359"} style={styles.root}>
      <Stack.Screen options={{ headerTitle: reward.name, headerBackButtonDisplayMode: 'minimal', 
